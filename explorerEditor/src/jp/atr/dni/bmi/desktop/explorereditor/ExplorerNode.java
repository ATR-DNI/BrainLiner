...
public class ExplorerNode extends AbstractNode {
    ...
    public Action[] getActions(boolean popup) {
        ...
        return new Action[]{
                    ...
                    null, //Line.
                    new ConvertToNeuroshareAction(), // Convert To Neuroshare
                    null, //Line.
    ...
    private ArrayList<Sheet.Set> setNeuroshareProperties(GeneralFileInfo obj) {
        ...
        try {
            ...
            for (int ii = 0; ii < entities.size(); ii++) {
                ...
                entityGrp.setDisplayName("Entity No : " + (ii + 1) + " : Type [" + entity.getEntityInfo().getEntityTypeLabel() + "]");
                ...
                PropertySupport.Reflection entityTypeProp = new PropertySupport.Reflection(entityInfo, String.class, "getEntityTypeLabel", null);
                ...
                if (entityType == EntityType.ENTITY_EVENT) {
                    EventInfo eventInfo = (EventInfo) entity;
                    PropertySupport.Reflection ei_EventTypeProp = new PropertySupport.Reflection(eventInfo, String.class, "getEventTypeLabel", null);
                    PropertySupport.Reflection ei_CSVDescProp = new PropertySupport.Reflection(eventInfo, String.class, "csvDesc");
                    ei_EventTypeProp.setName("Event Type"); // need to be identified.
                    ei_CSVDescProp.setName("CSV Desc"); // need to be identified.
                    ei_EventTypeProp.setShortDescription("type of the event. <i><B>ns_EventInfo.dwEventType.</B></i> <font color='red'><B>Unable to modify value.</B></font>");
...
//                        for (int jj = 0; jj < segSourceInfos.size(); jj++) {
    ...
    // Right-clicked menu. Convert to Neuroshare
    private class ConvertToNeuroshareAction extends AbstractAction implements Presenter.Popup {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            if (obj.getFileType() == FileType.NSN) {
                JOptionPane.showMessageDialog(null, "The file is Neuroshare.");
                return;
            }

            // Convert data file to Neuroshare.
            GeneralFileInfo gfi = new GeneralFileInfo(obj.getFilePath());
            // Temp file
            // File tempFile = new File("." + File.separator + "workingfiles" + File.separator + gfi.getFileName() + ".nsn");
            File tempFile = new File(gfi.getFilePath() + ".nsn");
            try {
                if (tempFile.exists()) {
                    tempFile.delete();
                }
    ...
    private class CopyToWorkspaceAction extends AbstractAction implements Presenter.Popup {
        ...
        public void actionPerformed(ActionEvent e) {
            ...
            // Workspace only allows Neuroshare format as a data file.
            if (obj.getFileType() == FileType.PLX || obj.getFileType() == FileType.NEV || obj.getFileType() == FileType.NSX || obj.getFileType() == FileType.CSV) {
                JOptionPane.showMessageDialog(null, "Select a Neuroshare file.");
                return;
            }
